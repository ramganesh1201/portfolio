...
export function CaseStudyChallenges({ project }: { project: CaseStudyData }) {
  return (
    <section className="py-16 px-6 md:px-10 relative z-20 w-full max-w-5xl mx-auto border-t border-white/[0.05]">
      <FadeIn y={20}>
        <h2 className="text-[#D7E2EA] font-bold text-3xl md:text-4xl uppercase tracking-wider mb-12">Challenges & Solutions</h2>
      </FadeIn>
      
      <div className="space-y-8">
        {project.challenges.map((item, index) => (
          <FadeIn key={index} delay={index * 0.1} y={20}>
            <div className="bg-[#0C0C0C] border border-white/[0.1] p-8 md:p-10 rounded-[30px] flex flex-col md:flex-row gap-8">
